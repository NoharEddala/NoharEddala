# Hi, I’m Nohar 👋

Electronics Engineer focused on Circuit Development, PCB design,  power electronics, and Hardware development.    

- 📚 MSc Renewable Energy & E-Mobility (Hochschule Stralsund)  
- 🛠 Skills: Analog / Digital circuit design, Schematic design, PCB design (Altium Designer, KiCad), Power Electronics, BMS, Power circuits, C/C++, Embedded (ESP32/STM32)   
- 🔭 Current: Loking for a full time job opportunity in the field of developing hardware electronics across Germany.  
- 📫 Contact: nohar.eddala@outlook.com  
- 🔗 LinkedIn: https://www.linkedin.com/in/nohar-chandrakanth-eddala-32b4b9180/

## Featured projects
- **Electronic Load for Fuel Cell Testing** — custom load & firmware for 160 A stack evaluation  
- **Redox Flow Battery Test Bench** — Experimental test platform for battery stack evaluation and performance testing 
- **IoT Smart Grid Monitoring** — real-time fault detection and transformer health tracking

*(See my repositories below for schematics, firmware, and documentation.)*
